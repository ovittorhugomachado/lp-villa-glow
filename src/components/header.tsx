  ...
  return (
    <header className="w-screen h-18 flex items-center justify-between bg-black/50 fixed top-0 px-4 md:px-12 py-3 z-99">
      <button
        ...
        onClick={() => handleScroll("hero-fold")}
      >
        <h2 className="text-3xl md:text-4xl translate-y-2 text-[#C3973B]">
          Villa Glow
        </h2>
      ...
      <div className="flex items-center gap-2">
        ...
        <nav className="hidden md:block">
          <ul className="flex gap-4 items-start font-semibold text-white pl-2">
            <li
              onClick={() => handleScroll("first-fold", -100)}
              className="cursor-pointer"
            >
              Experiência
            </li>
            <li
              onClick={() => handleScroll("second-fold")}
              className="cursor-pointer"
            >
              Informações
            </li>
            <li
              onClick={() => handleScroll("third-fold")}
              className="cursor-pointer"
            >
              Localização
            </li>
      ...
      {openMenu && (
        <div className="w-screen md:hidden h-screen top-0 left-0 fixed bg-black/50" />
      )}
      <nav
        className={`${openMenu ? "" : "translate-x-50"} w-[180px] md:hidden h-screen pt-5 px-3 bg-[#0A0533] fixed top-0 right-0 transition-all duration-500`}
      >
        <button
